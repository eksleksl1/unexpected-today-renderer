...
def main(job_path):
    job=json.loads(Path(job_path).read_text(encoding="utf-8"));WORK.mkdir(exist_ok=True);OUT.mkdir(exist_ok=True);frames=prepare(job.get("image_urls") or []);paragraphs=[x.strip() for x in re.split(r"\n\s*\n",job["script"]) if x.strip()];texts=paragraphs or split_count(job["script"],max(len(frames),6));captions=job.get("captions") or [];scene_frames=job.get("scene_frames") or [];parts=[]
    for i,text in enumerate(texts):
        mapped=scene_frames[i] if i<len(scene_frames) and isinstance(scene_frames[i],int) else int(i*len(frames)/len(texts));frame=frames[min(len(frames)-1,max(0,mapped))];base=WORK/f"part-{i:03d}";png=base.with_suffix(".png");mp3=base.with_suffix(".mp3");vtt=base.with_suffix(".vtt");sub=base.with_suffix(".ass");mp4=base.with_suffix(".mp4");slide(job["title"],png,frame);asyncio.run(voice(text,mp3,vtt));length=duration(mp3)+.25;caption_ass(captions[i] if i<len(captions) else text,length,sub);subtitle_filter=f"subtitles={sub}," if frame["caption"] else ""
